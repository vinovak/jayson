 ...
 *  @api public
 */
Server.prototype.call = function(request, req, originalCallback) {
  var self = this;
  ...
  maybeParse(request, this.options, function(err, request) {
    ...
    if(request.params && !Array.isArray(request.params)) {
      ...
      // pop another one, because it must be the full original request object passed to the middleware
      parameters.pop();
    ...
    // the full original request object passed to the middleware
    args.push(req);
